    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card-box page-title-box">
                <h4 class="header-title">Update Exclude Url</h4>
            </div>
        </div>
    </div>
    <form id="ShopLeadFormUpdateEx" method="POST" action="{{ route('ExcludeUrlUpdate') }}">
        {{ csrf_field() }}
        <div class="row">
            <div class="col-sm-6">
            ...
            <div class="col-sm-12">
                <div class="form-group">
                    ...
                    <textarea class="form-control rounded-0" name="UrlList" id="SourcesList" required rows="1">{{implode(",", json_decode($campaign_Data->exclude_url))}}</textarea>
                </div>
            </div>
        </div>
        <div class="row">
