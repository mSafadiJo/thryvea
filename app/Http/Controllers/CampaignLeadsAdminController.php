...
class CampaignLeadsAdminController extends Controller
{
    ...
    public function fetch_data_lead_received(Request $request)
    {
        ...
        $sort_by = str_replace('-', '.', $request->get('sortby'));
        ...
        if (!empty($query_search)) {
            $idQuery->where(function ($query) use ($query_search) {
                $query->where('leads_customers.lead_id', 'like', '%' . $query_search . '%')
                    ->orWhere('campaigns_leads_users.campaigns_leads_users_id', 'like', '%' . $query_search . '%')
                    ->orWhere('leads_customers.lead_fname', 'like', '%' . $query_search . '%')
                    ->orWhere('leads_customers.lead_lname', 'like', '%' . $query_search . '%')
                    ->orWhereRaw("concat(leads_customers.lead_fname, ' ', leads_customers.lead_lname) like ?", ["%" . $query_search . "%"])
                    ->orWhere('leads_customers.lead_source_text', 'like', '%' . $query_search . '%')
        ...
        if (!empty($seller_id)) {
            $idQuery->join('campaigns AS camp_seller', 'camp_seller.vendor_id', '=', 'leads_customers.vendor_id')
                ->whereIn('camp_seller.user_id', $seller_id);
        }

        if (!empty($service_id)) {
            $idQuery->whereIn('campaigns.service_campaign_id', $service_id);
        }
    ...
    public function list_of_leads_lost()
    {
        ...
        // STEP 1: Get IDs only
        $idQuery = DB::table('leads_customers')
            ->select('lead_id')
        ...
        $paginatedIds = $idQuery->simplePaginate(10);
        $ids = collect($paginatedIds->items())->pluck('lead_id')->toArray();
    ...
    public function fetch_data_lead_Lost(Request $request)
    {
        ...
        // STEP 1: Build ID query
        $idQuery = DB::table('leads_customers')
            ->select('leads_customers.lead_id')
            ->whereNotExists(function ($sub) {
                $sub->select(DB::raw(1))
                    ->from('campaigns_leads_users')
                    ->whereColumn('campaigns_leads_users.lead_id', 'leads_customers.lead_id');
        ...
        // Environment filters
        if (!empty($environments) && $environments == 1) {
            $idQuery->where('leads_customers.is_duplicate_lead', '<>', 1)
        ...
        } elseif (!empty($environments) && $environments == 3) {
            $idQuery->where('leads_customers.is_duplicate_lead', 1);
        } elseif (!empty($environments) && $environments == 7) {
            $idQuery->where('leads_customers.is_duplicate_lead', "<>", 1)
                ->where('leads_customers.status', 2);
        } elseif (!empty($environments) && $environments == 9) {
            $idQuery->where('leads_customers.is_duplicate_lead', "<>", 1)
            ...
            $idQuery->where('leads_customers.lead_fname', '!=', "test")
                ...
                ->where('leads_customers.lead_lname', '!=', "Test")
                ->where('leads_customers.is_test', 0);
        } elseif (!empty($environments) && $environments == 10) {
            $idQuery->where('leads_customers.is_duplicate_lead', "<>", 1)
        ...
        // Seller filter requires join — only add if needed
        if (!empty($seller_id)) {
            $idQuery->join('campaigns', 'campaigns.vendor_id', '=', 'leads_customers.vendor_id')
                ->whereIn('campaigns.user_id', $seller_id);
        }
    ...
    public function fetch_data_lead_Affiliate(Request $request)
    {
        ...
        // ============================================================
        // STEP 1: Build ID query with all filters
        // ============================================================
        $idQuery = DB::table('leads_customers')->select('leads_customers.lead_id');
        ...
        if (!empty($query_search)) {
            $idQuery->where(function ($query) use ($query_search) {
                $query->where('leads_customers.lead_id', 'like', '%' . $query_search . '%')
                    ->orWhere('leads_customers.lead_fname', 'like', '%' . $query_search . '%')
                    ->orWhere('leads_customers.lead_lname', 'like', '%' . $query_search . '%')
                    ->orWhere('leads_customers.response_data', 'like', '%' . $query_search . '%')
                    ->orWhereRaw("concat(leads_customers.lead_fname, ' ', leads_customers.lead_lname) like ?", ["%" . $query_search . "%"])
                    ->orWhere('leads_customers.lead_source_text', 'like', '%' . $query_search . '%');
            });
        }
...
//    }
    ...
    public function export_lead_data(Request $request)
    {
        ...
        if (!empty($allLead)) {
            ...
            // Build base query
            $query = $this->buildLeadQuery($request);

            // Apply filters
            $this->applyLeadFilters($query, $request);

            // Apply environment-specific constraints
            $this->applyEnvironmentConstraints($query, $request->environments);

            // Fetch data
            $lead_data = $this->fetchLeadData($query, $request->environments);

            // Log access
            $this->logExportAccess($type_name, $request);

            // Export
            return $this->exportToCsv($lead_data, $request->environments);
        }
    }
...
// -------------------------------------------------------------------------
    ...
    private function buildLeadQuery(Request $request): \Illuminate\Database\Query\Builder
    {
        // CRITICAL FIX: was `if($environments = 2)` (assignment, always true)
        if ($request->environments == 2) {
            return DB::table('campaigns_leads_users')
                ->join('campaigns', 'campaigns.campaign_id', '=', 'campaigns_leads_users.campaign_id')
                ->join('service__campaigns', 'service__campaigns.service_campaign_id', '=', 'campaigns.service_campaign_id')
                ->join('leads_customers', 'leads_customers.lead_id', '=', 'campaigns_leads_users.lead_id')
                ->leftJoin('campaigns AS camp_seller', 'camp_seller.vendor_id', '=', 'leads_customers.vendor_id')
        ...
        return DB::table('leads_customers')
            ->join('service__campaigns', 'service__campaigns.service_campaign_id', '=', 'leads_customers.lead_type_service_id')
            ->join('states', 'states.state_id', '=', 'leads_customers.lead_state_id')
            ->join('counties', 'counties.county_id', '=', 'leads_customers.lead_county_id')
            ->join('cities', 'cities.city_id', '=', 'leads_customers.lead_city_id')
    ...
    {
        $filters = [
            'leads_customers.lead_type_service_id' => $this->parseArrayInput($request->service_id),
            'leads_customers.lead_state_id' => $this->parseArrayInput($request->states),
            'leads_customers.lead_county_id' => $this->parseArrayInput($request->county_id),
            'leads_customers.lead_city_id' => $this->parseArrayInput($request->city_id),
            'leads_customers.lead_zipcode_id' => $this->parseArrayInput($request->zipcode_id),
            'leads_customers.google_ts' => $this->parseArrayInput($request->traffic_source),
            'leads_customers.google_g' => $this->parseArrayInput($request->google_g),
            'leads_customers.google_c' => $this->parseArrayInput($request->google_c),
            'leads_customers.google_k' => $this->parseArrayInput($request->google_k),
            'campaigns_leads_users.user_id' => $this->parseArrayInput($request->buyer_id),
        ];
        ...
        // File-based filters (zipcodes, counties, cities)
        $this->applyFileListFilter($query, $request->file('listOfzipcode'), 'zip_codes_lists.zip_code_list');
        $this->applyFileListFilter($query, $request->file('ListOfCountiesFile'), 'counties.county_name', true);
        $this->applyFileListFilter($query, $request->file('ListOfCitiesFile'), 'cities.city_name', true);

        // Date range
        if ($request->filled(['start_date', 'end_date'])) {
            $query->whereBetween('leads_customers.created_at', [
    ...
    private function applyFileListFilter($query, $file, string $column, bool $useLike = false): void
    {
        if (!$file) {
            return;
        }

        $values = collect(Excel::toArray(new SalesOrder, $file)[0] ?? [])->pluck(0)->filter()->values()->all();
    ...
    private function applyEnvironmentConstraints($query, int $environment): void
    {
        // Base exclusions for test leads (reused across multiple environments)
        $excludeTests = function ($q) {
            $q->where('leads_customers.lead_fname', '!=', 'test')
                ->where('leads_customers.lead_lname', '!=', 'test')
                ->where('leads_customers.lead_fname', '!=', 'testing')
                ->where('leads_customers.lead_lname', '!=', 'testing')
                ->where('leads_customers.lead_fname', '!=', 'Test')
        ...
        $constraints = [
            3 => function ($q) use ($excludeTests) {
                $q->whereNull('campaigns_leads_users.lead_id')
                    ->where('leads_customers.status', 0)
                    ->where($excludeTests)
                    ->where('leads_customers.is_duplicate_lead', '<>', 1);
            },
            4 => function ($q) {
                $q->where('leads_customers.status', 1)
                    ->where('leads_customers.is_duplicate_lead', '<>', 1);
            },
            5 => function ($q) {
                $q->where(function ($sub) {
                    $sub->where('leads_customers.lead_fname', 'test')
                        ->orWhere('leads_customers.lead_lname', 'test')
                        ->orWhere('leads_customers.lead_fname', 'testing')
                        ->orWhere('leads_customers.lead_lname', 'testing')
                        ->orWhere('leads_customers.lead_fname', 'Test')
            ...
            6 => function ($q) {
                $q->where('leads_customers.is_duplicate_lead', 1);
            },
            7 => function ($q) {
                $q->where('leads_customers.is_duplicate_lead', '<>', 1)
                    ->where('leads_customers.status', 2);
            },
            8 => function ($q) use ($excludeTests) {
                $q->whereNotNull('campaigns_leads_users.lead_id')
                    ->whereNotIn('leads_customers.status', [1, 2])
                    ->where('campaigns_leads_users.is_returned', 1)
                    ->where($excludeTests)
                    ->where('leads_customers.is_duplicate_lead', '<>', 1);
            },
            9 => function ($q) use ($excludeTests) {
                $q->whereNull('campaigns_leads_users.lead_id')
                    ->where('leads_customers.status', 3)
                    ->where($excludeTests)
                    ->where('leads_customers.is_duplicate_lead', '<>', 1);
            },
            10 => function ($q) use ($excludeTests) {
                $q->whereNull('campaigns_leads_users.lead_id')
                    ->where('leads_customers.status', 4)
                    ->where($excludeTests)
                    ->where('leads_customers.is_duplicate_lead', '<>', 1);
        ...
        if (isset($constraints[$environment])) {
            $constraints[$environment]($query);
        } else {
            // Default environment
            $query->where('leads_customers.is_duplicate_lead', '<>', 1)
                ->where(function ($q) {
                    $q->whereNull('campaigns_leads_users.is_returned')
                        ->orWhere('campaigns_leads_users.is_returned', 0);
                })
    ...
    private function fetchLeadData($query, int $environment)
    {
        if ($environment == 2) {
            return $query->orderBy('leads_customers.created_at', 'DESC')
                ->groupBy('campaigns_leads_users.campaigns_leads_users_id')
                ->get([
                    'campaigns_leads_users.campaigns_leads_users_id',
                    'campaigns.campaign_name',
                    ...
                    'zip_codes_lists.zip_code_list',
                    'campaigns_leads_users.created_at AS sold_date',
                    'campaigns_leads_users.transactionId',
                    'campaigns_leads_users.campaigns_leads_users_note'
                ]);
        }
    ...
    private function logExportAccess(string $type_name, Request $request): void
    {
        AccessLog::create([
            'user_id' => Auth::id(),
            'user_name' => Auth::user()->username,
    ...
    private function getInfoPermission(): int
    {
        $permission_users = json_decode(Auth::user()->permission_users ?? '[]', true);
        return (empty($permission_users) || in_array('8-23', $permission_users)) ? 1 : 0;
    }

    private function exportToCsv($lead_data, int $environment)
    {
        $info_permission = $this->getInfoPermission();

        if ($environment == 2) {
            return (new FastExcel($lead_data))->download('leads.csv', function ($lead) use ($info_permission) {
                return $this->mapCampaignColumns($lead, $info_permission);
            });
        }

        return (new FastExcel($lead_data))->download('leads.csv', function ($lead) use ($info_permission) {
            return $this->mapLeadColumns($lead, $info_permission);
        });
    }

    private function mapCommonColumns($lead, int $info_permission): array
    {
        return [
            'First Name' => $lead->lead_fname,
            'Last Name' => $lead->lead_lname,
            'Email Address' => $info_permission ? $lead->lead_email : '',
            'Phone Number' => $info_permission ? ' ' . $lead->lead_phone_number . ' ' : '',
            'Service' => $lead->service_campaign_name,
            'Address' => $info_permission ? $lead->lead_address : '',
            'City' => $lead->city_name,
            'County' => $lead->county_name,
            'State' => $lead->state_code,
            'ZIPCode' => ' ' . $lead->zip_code_list . ' ',
            ...
            'Details' => $lead->lead_details_text,
            'Jornaya Id' => $lead->universal_leadid,
            'TrustedForm URL' => $info_permission ? $lead->trusted_form : '',
            'TS' => $lead->google_ts,
            'C' => $lead->google_c,
            'K' => $lead->google_k,
            'G' => $lead->google_g,
    ...
    private function mapCampaignColumns($lead, int $info_permission): array
    {
        return array_merge([
            'ID' => $lead->campaigns_leads_users_id,
            'Lead Id' => $lead->lead_id,
        ], $this->mapCommonColumns($lead, $info_permission), [
            'Bid' => $lead->campaigns_leads_users_bid,
            'Type' => $lead->campaigns_leads_users_type_bid,
            'Buyer' => $lead->user_business_name,
            'Campaign' => $lead->campaign_name,
    ...
    private function mapLeadColumns($lead, int $info_permission): array
    {
        $status = !empty($lead->sum_bid)
            ? ($lead->is_returned_concat == 1 ? 'Returned' : 'Sold')
            : 'Unsold';
