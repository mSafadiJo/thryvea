...
//                }
    ...
    public function service_queries_all($service, $LeaddataIDs,  $address, $lead_source = "All Source", $is_forms = 0, $sub_id='', $OriginalURL=''){
        $projectnatureArrayData = array();
        if( empty($LeaddataIDs['project_nature']) ){
            $projectnatureArrayData[] = 1;
            $projectnatureArrayData[] = 2;
        ...
        // Replicate exactly what each original query returned
        $exclusive_ids = $allCampaigns
            ->filter(function($c) {
                if (empty($c->custom_paid_campaign_id)) return false;
                $ids = json_decode($c->custom_paid_campaign_id, true);
                return is_array($ids)
                    && in_array('1', $ids)
                    && $c->campaign_budget_bid_exclusive != 0
                    && !$c->is_ping_account;
            })
            ->sortByDesc('campaign_budget_bid_exclusive')
            ->values();

        $shared_ids = $allCampaigns
            ->filter(function($c) {
                if (empty($c->custom_paid_campaign_id)) return false;
                $ids = json_decode($c->custom_paid_campaign_id, true);
                return is_array($ids)
                    && in_array('2', $ids)
                    && $c->campaign_budget_bid_shared != 0
                    && !$c->is_ping_account;
            })
            ->sortByDesc('campaign_budget_bid_shared')
            ->values();

        $ping_exclusive = $allCampaigns
            ->filter(function($c) {
                if (empty($c->custom_paid_campaign_id)) return false;
                $ids = json_decode($c->custom_paid_campaign_id, true);
                return is_array($ids)
        ...
        return [
            'exclusive' => $exclusive_ids,
            'shared'    => $shared_ids,
            'ping_ex'   => $ping_exclusive,
            'ping_sh'   => $ping_shared,
        ];
    }
